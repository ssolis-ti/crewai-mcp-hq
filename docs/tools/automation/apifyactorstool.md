...
The `ApifyActorsTool` requires these inputs to work:
  ...
  * **`actor_name`** The ID of the Apify Actor to run, e.g., `"apify/rag-web-browser"`. Browse all Actors on [Apify Store](https://apify.com/store).
  * **`run_input`** A dictionary of input parameters for the Actor when running the tool manually.
...
[OverviewPrevious](/en/tools/automation/overview)[Composio ToolNext](/en/tools/automation/composiotool)
