...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "4. PREPARE: the project's .env needs real LLM API keys (e.g. "
        "OPENAI_API_KEY) — ask the user to set them; you cannot. Then run "
        "crewai_install_deps(project).\n"
        "5. RUN: crewai_kickoff(project, inputs={...}). The inputs keys must "
        ...
        "6. DEBUG & IMPROVE: if a run fails, read the returned STDERR, fix "
