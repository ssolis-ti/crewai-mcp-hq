# Source: https://docs.crewai.com/en/enterprise/guides/vertex-ai-workload-identity-setup
...
The crew SDK reads these like any other env var — no code changes required, provided your crew was deployed against **`crewai>=1.14.3`** (see Version requirements).
...
Run a crew that uses the Vertex connection and tail the execution logs in AMP. A successful `generateContent` or `predict` call confirms the full chain — OIDC mint → STS exchange → SA impersonation → Vertex — is wired correctly. If the crew fails, see Troubleshooting below. Most issues trace back to the GCP-side configuration — the OIDC provider’s `attribute-condition` or the service account’s `principalSet` binding.
...
[Azure OpenAI SetupPrevious](/en/enterprise/guides/azure-openai-setup)[Tool RepositoryNext](/en/enterprise/guides/tool-repository)
