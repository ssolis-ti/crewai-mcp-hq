...
Because memory persists across runs (backed by LanceDB on disk), the `analyze` step will recall findings from previous executions too — enabling flows that learn and accumulate knowledge over time. See the [Memory documentation](/concepts/memory) for details on scopes, slices, composite scoring, embedder configuration, and more.
...
[CrewsPrevious](/en/concepts/crews)[Production ArchitectureNext](/en/concepts/production-architecture)
