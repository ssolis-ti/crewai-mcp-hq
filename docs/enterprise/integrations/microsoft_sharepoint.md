...
**Description:** Append a new row to the end of an Excel table in a SharePoint file. The values array must have the same number of elements as the table has columns.**Parameters:**
  ...
  * `values` (array, required): Array of cell values for the new row, one per column in table order (e.g., [“John Doe”, “[john@example.com](mailto:john@example.com)”, 25])
...
[Microsoft Outlook IntegrationPrevious](/en/enterprise/integrations/microsoft_outlook)[Microsoft Teams IntegrationNext](/en/enterprise/integrations/microsoft_teams)
